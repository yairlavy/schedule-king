# tests/tests_models/test_generate_metrics.py
...
# Updated Lecture to include end_time
Lecture = namedtuple("Lecture", ["day", "start_time", "end_time"])
...
def single_day_schedule():
    lg1 = MockLectureGroup(lecture=[Lecture(day="2", start_time=time(9, 0), end_time=time(10, 0))])
    lg2 = MockLectureGroup(tirguls=[Lecture(day="2", start_time=time(10, 0), end_time=time(11, 0))])
    lg3 = MockLectureGroup(maabadas=[Lecture(day="2", start_time=time(11, 0), end_time=time(12, 0))])
    return Schedule(lecture_groups=[lg1, lg2, lg3])

@pytest.fixture
def multi_day_schedule_with_gaps():
    lg1 = MockLectureGroup(lecture=[Lecture(day="2", start_time=time(9, 0), end_time=time(10, 0))])
    lg2 = MockLectureGroup(tirguls=[Lecture(day="2", start_time=time(11, 0), end_time=time(12, 0))])
    lg3 = MockLectureGroup(lecture=[Lecture(day="4", start_time=time(8, 0), end_time=time(9, 0))])
    lg4 = MockLectureGroup(tirguls=[Lecture(day="4", start_time=time(9, 0), end_time=time(10, 0))])
    lg5 = MockLectureGroup(maabadas=[Lecture(day="4", start_time=time(13, 0), end_time=time(14, 0))])
    return Schedule(lecture_groups=[lg1, lg2, lg3, lg4, lg5])

@pytest.fixture
def schedule_with_minimal_gap():
    lg1 = MockLectureGroup(lecture=[Lecture(day="3", start_time=time(9, 0), end_time=time(10, 0))])
    lg2 = MockLectureGroup(tirguls=[Lecture(day="3", start_time=time(10, 30), end_time=time(11, 30))])
    return Schedule(lecture_groups=[lg1, lg2])

@pytest.fixture
def schedule_with_varied_days_and_empty_groups():
    lg1 = MockLectureGroup()
    lg2 = MockLectureGroup(lecture=[Lecture(day="1", start_time=time(8, 0), end_time=time(9, 0))])
    lg3 = MockLectureGroup(tirguls=[Lecture(day="5", start_time=time(14, 0), end_time=time(15, 0))])
    return Schedule(lecture_groups=[lg1, lg2, lg3])
...
class TestScheduleGenerateMetrics:
    ...
    def test_lectures_on_all_days(self):
        lgs = [
            MockLectureGroup(lecture=[Lecture(day=str(i), start_time=time(7 + i, 0), end_time=time(8 + i, 0))])
            for i in range(1, 8)
        ]
    ...
    def test_lectures_with_nonstandard_day_names(self):
        lg = MockLectureGroup(lecture=[Lecture(day="X", start_time=time(10, 0), end_time=time(11, 0))])
    ...
    def test_lectures_with_overlapping_times(self):
        lg1 = MockLectureGroup(lecture=[Lecture(day="2", start_time=time(9, 0), end_time=time(10, 0))])
        lg2 = MockLectureGroup(tirguls=[Lecture(day="2", start_time=time(9, 30), end_time=time(10, 30))])
